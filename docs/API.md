...
```json
{ "success": false, "error": { "status": 400, "code": "USER006", "message": "Email does not exist" } }
```
...
```json
{ "success": true, "data": { "data": [ /* rows */ ], "total": 0, "totalPages": 0, "page": 1, "limit": 20 } }
```
...
### Orders (HMAC, server-to-server — **not a browser flow**)

| Method | Path | Auth | Note |
|---|---|---|---|
| POST | `/api/orders` | 🔓* | `{amount,coin,network,externalOrderId}` → `{order:{orderId}}` |
| GET | `/api/orders?page&limit&status` | 🔓* | list |
| GET | `/api/orders/:orderId` | 🔓* | one order |

\* `noauth` in Postman but authenticated by **HMAC** with the integration's
API key pair (signed server-side by the merchant's backend). The merchant
**checkout widget** consumes order *state*, but order creation is
server-to-server — do not call from the browser with the user session.
...
route guard, signed-in header menu, i18n en/fr.

**Not yet wired:** everything in Fund, Affiliate, Integrations, plus
verify-email / forgot-password / reset-password / change-password / 2FA,
and the Orders checkout widget.
...
2. **Merchant dashboard – Integrations** (CRUD + API keys): real data for
   the dashboard sprint; paginated list shape already known. Mind the
   "secret shown once" UX for `ipnSecret` / `privateKey`.
3. **Fund**: balance + deposit address (`get-address`), then histories
   (paginated), then withdraw (multi-step: request → email approval).
4. **Affiliate dashboard**: `totals` + `downline`/`commissions` (paginated).
5. **Checkout widget** (Orders): order *state* view — coordinate with
   backend (HMAC is server-to-server; the browser only reads order status).
