...
- Setup i18n (en/fr — mặc định en; Vietnamese đã loại bỏ)
...
- **Merchant App:** Sans `Geist Sans` + Mono `Geist Mono` (Vercel, gói `geist`,
  self-host qua `next/font`) — Clear/Clean/Luxurious, hợp vibe AI-native. Geist
  Mono cho dữ liệu crypto (địa chỉ ví, tx hash, số tiền).
- **Admin / shared default:** `Inter` + `JetBrains Mono` (token mặc định trong
  `packages/config/tailwind/theme.css`).
- Merchant override token `--font-sans`/`--font-mono` cục bộ trong
  `apps/merchant-app/src/app/globals.css` — không đụng theme dùng chung.
...
- Default locale: `en`
- Messages: `src/i18n/messages/{en,fr}.json` (Vietnamese đã loại bỏ hoàn toàn)
...
- 2.  Integrate widget or API
- 3.  Receive crypto, auto-settle
...
- Comment tiếng Anh, message i18n theo locale (English/French)
